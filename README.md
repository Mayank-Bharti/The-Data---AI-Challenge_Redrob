...
**🔗 [Run the HDE-CM Live Inference Engine on Google Colab Here](https://colab.research.google.com/drive/1QKydgpojM4Pj8BpdtNjeomtb9fllfypN?authuser=2)**
...
_(Note: The `precomputed_features.pkl` file is a Python "Pickle" database. It contains the mathematical vector embeddings of all the candidates generated by the AI model. By loading this serialized file directly into RAM, our ranking engine can instantly run complex mathematical comparisons without having to re-process the massive raw text dataset.)_
...
2. **(Optional) Stage 1 - Precompute (Offline):**
   _If you want to generate the embeddings from scratch, place the `candidates.jsonl` file in the `/datasets` folder._

   ```bash
   python precompute.py
   ```

   _This generates the `precomputed_features.pkl` file._

3. **Stage 2 - Rank & Output (Online):**
   _If you downloaded the `.pkl` file from Google Drive, place it in the root directory and you can instantly run the ranking engine._

   ```bash
   python ranker.py
   ```

   _This evaluates the candidates in ~5 seconds and generates `The_Crimson_Codex.csv`._
